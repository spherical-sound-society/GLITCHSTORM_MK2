...
ISR(TIMER1_COMPA_vect) {
  ...
  switch (programNumber) {
    ...
    case 16:
      ...
      //DEFFO t*(t&16384?6:5)*(4-(1&t>>8))>>(3&-t>>9)|t>>6
  ...
  if (cyclebyte == 1024 ) {
    cyclebyte = 0;
    clockOut0bit = !clockOut0bit;
    digitalWrite(clockOut0, clockOut0bit);
    if (isSequencing) {
      seqLooper++;
      if (seqLooper == 16) {
        seqLooper = 0;
        modifySequence(sequence, 16);
      }
      t = sequence[seqLooper];
    }
...
void buttonsManager() {
  ...
  if (digitalRead(leftButtonPin) == LOW) {
    ...
    if ((millis() - button1Timer > longPress1Time) && (isLongPress1Active == false)) {
      ...
      Serial.println("Toggle sequencing");
    }
  } else {
