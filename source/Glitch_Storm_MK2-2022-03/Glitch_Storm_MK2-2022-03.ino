// Glitch Storm MK2 rev 0.1
// CC By Sa Spherical Sound Society 2021
...
void setup() {
  ...
  pinMode(clockOut0, OUTPUT);
  pinMode(clockOut1, OUTPUT);
  ...
  initSequence(sequence, 16, 65535 );
}
...
void sampleRateManager() {

  // SAMPLE RATE *************************************

  old_SAMPLE_RATE = SAMPLE_RATE;
  //int actual_SAMPLE_RATE = analogRead(1);
  SAMPLE_RATE = softDebounce(analogRead(3), SAMPLE_RATE);
...
void initSequence( unsigned int seq[], byte sizeOfArray, unsigned int variation ) {
  for ( int i = 0 ; i < sizeOfArray ; i++ ) {
    seq[ i ] = random(variation);
    Serial.print(seq[ i ]);
  }
}
void modifySequence( unsigned int seq[], byte sizeOfArray ) {
  for ( int i = 0 ; i < sizeOfArray ; i++ ) {
    if (random(255) < 128) {
      int r = random(16384);
      seq[ i ] = seq[ i ] + r -8192;
